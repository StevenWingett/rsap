...
coldata <- read.csv(metadata_file, sep="\t", row.names=1, check.names = FALSE)
#head(coldata)
#class(coldata)
...
outfile <- paste0(outdir, '/', comparison, '.meanSdPlot_vsd.svg')
...
# PCA
outfile <- paste0(outdir, '/', comparison, '.pca.png')
...
outfile <- paste0(outdir, '/', comparison, '.distanceMatrixHeatmap.png')
